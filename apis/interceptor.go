...
func AddInterceptor(ctx *gin.Context) {
	...
	if err != nil {
		...
		ctxs.FailHttpResp(ctx, errs.AdminErrorCode_Default)
		return
	}
...
func DeleteInterceptor(ctx *gin.Context) {
	...
	// clean dangling condition rows to keep data consistent
...
func ListInterceptors(ctx *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			ctxs.SuccessHttpResp(ctx, map[string]interface{}{"items": []*InterceptorReq{}})
			return
		}
		logs.NewLogEntity().Error(err.Error())
		ctxs.FailHttpResp(ctx, errs.AdminErrorCode_Default)
	...
	ctxs.SuccessHttpResp(ctx, map[string]interface{}{"items": resp})
}
