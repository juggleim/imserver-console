...
func (ext UserExtDao) QryExtFieldsByItemKeys(appkey, userId string, itemKeys []string) (map[string]*UserExtDao, error) {
	var items []*UserExtDao
	err := dbcommons.GetDb().Where("app_key=? and user_id=? and item_key in (?)", appkey, userId, itemKeys).Find(&items).Error
	ret := map[string]*UserExtDao{}
	for _, item := range items {
		ret[item.ItemKey] = &UserExtDao{
			ID:          item.ID,
			UserId:      item.UserId,
